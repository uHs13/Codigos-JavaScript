class Files {
    ...
    constructor(fb) {
        ...
        this._progressBarEl = document.querySelector(".mc-progress-bar-fg");
        this._pbFilenameEl = document.querySelector(".filename");
        this._breadCrumbEl = document.querySelector("#browse-location");
        this.breadCrumbLinksEl = "";
        this._uploadTimeStart = "";
    ...
    set pbFilenameEl(value) {
        this._pbFilenameEl.innerHTML = value;
    }

    get uploadTimeStart() {
        return this._uploadTimeStart;
    }

    set uploadTimeStart(value) {
        this._uploadTimeStart = value;
    }

    get breadCrumbEl() {
        return this._breadCrumbEl;
    }

    set breadCrumbEl(value) {
        this._breadCrumbEl.innerHTML = value;
    }
    ...
    openFolder() {
        ...
        if (this.lastFolder) this.fb.firebaseDatabaseRef.ref(this.lastFolder).off("value");
    ...
    updateBreadCrumb() {
        ...
        [...this.fb.currentFolder].forEach((f, index) => {

            path.push(f);

            let span = document.createElement("span");

            //último elemento do breadCrumb
            if ((index + 1) === this.fb.currentFolder.length) {
                ...
                <span class="ue-effect-container uee-BreadCrumbSegment-link-0">
                    <a href="#" data-path = "${path.join("/")}"class="breadcrumb-segment">${f}</a>
                </span>
        ...
        this.breadCrumbEl = nav.innerHTML;

        this.breadCrumbLinksEl = this.breadCrumbEl.querySelectorAll("a");

        this.setBreadCrumbLinksEvents();

    }
    //.updateBreadCrumb

    setBreadCrumbLinksEvents() {

        [...this.breadCrumbLinksEl].forEach(a => {

            a.addEventListener("click", e => {

                e.preventDefault();

                this.fb.currentFolder = a.dataset.path.split("/");
    ...
    getFileView(file, key) {
        ...
        window.dropBox.initEventsLi(li);

        return li;

        /*

            File {name: "IMG-20181220-WA0000.jpg", lastModified: 1545299886000, lastModifiedDate: Thu Dec 20 2018 07:58:06 GMT-0200 (Horário de Verão de Brasília), webkitRelativePath: "", size: 83723, …}
            lastModified: 1545299886000
            lastModifiedDate: Thu Dec 20 2018 07:58:06 GMT-0200 (Horário de Verão de Brasília) {}
            name: "IMG-20181220-WA0000.jpg"
            size: 83723
            type: "image/jpeg"
            webkitRelativePath: ""
    ...
    removeFile() {
        ...
        window.dropBox.getSelection().forEach(file => {

            //recuperando os dados do arquivo que estão salvos no dataset
            let arqFile = JSON.parse(file.dataset.file);

            let key = file.dataset.key;

            //                           route, method
            promises.push(new Promise((resolve, reject) => {

                if (arqFile.type === "folder") {

                    this.fb.removeFolder(this.fb.currentFolder.join("/"), arqFile.name).then(() => {
                ...
                } else if (arqFile.type) {

                    this.fb.removeFile(this.fb.currentFolder.join("/"), arqFile.name).then(() => {
            ...
            //envia os dados pra rota delete("/file") do arquivo index.js na pasta routes
    ...
    uploadTask(files) { // Recebe os arquivos que serão enviados para o servidor

        let promises = []; // Array que vai armazenar as promessas de todos os arquivos que vamos enviar para o servidor
        // cada arquivo no upload vai ter uma promessa própria que vai controlar se o upload ocorreu ou não

        //spread -> converte uma coleção em um array
        [...files].forEach(file => {

            promises.push(new Promise((resolve, reject) => {

                //Cria uma referência do arquivo dentro do storage do Firebase
                let filePointer = this.fb.firebaseStorage.ref(this.fb.currentFolder.join("/")).child(file.name);

                //Faz o upload do arquivo dentro da referência e retorna os eventos do upload
                let task = filePointer.put(file);

                //acompanha o processo de upload do arquivo no storage
                task.on("state_changed", snapShot => {
                    ...
                    filePointer.getMetadata().then(metadata => {
        ...
        return Promise.all(promises);//Recebe um array de promisses, executa e controla se tudo deu certo ou não. Se der errado ele indica qual promessa que não funcionou
        //executa todas as promises adicionadas na coleção
    }
    //.uploadTask

    modalShow(show = true) {

        window.dropBox.snackModalEl.style.display = (show) ? 'block' : 'none';
    ...
    uploadComplete() {
        ...
        /* Zera o campo para podermos escolher outro arquivo */
        window.dropBox.inputFileEl.value = "";

        Utils.swapbutton(window.dropBox.btnSendFileEl, false);
    ...
    buildFilesView() {
        ...
        this.lastFolder = this.fb.currentFolder.join("/");

        this.fb.firebaseDatabaseRef.ref(this.fb.currentFolder.join("/")).on("value", snapshot => {

            window.dropBox.listFilesEl.innerHTML = "";

            snapshot.forEach(snapItem => {

                if (snapItem.val().name && snapItem.val().path && snapItem.val().type) {
                    //caso não seja um arquivo, seja um nó de uma subpasta

                    let key = snapItem.key;

                    let data = snapItem.val();

                    window.dropBox.listFilesEl.appendChild(this.getFileView(data, key));
    ...
    updateUploadInfo(filename, percent) {

        let spentTime = Date.now() - this.uploadTimeStart;

        let forecast = ((100 - percent) * spentTime) / percent;

        this.pbTimeleftEl = Utils.formatTime(forecast);

        this.pbFilenameEl = filename;
    ...
    uploadProgress(event, file) {
        ...
        this.updateUploadProgressDesign(percent);

        this.updateUploadInfo(file.name, percent);

    }
    //.uploadProgress

    updateUploadProgressDesign(percent) {
    ...
    getFileIconView(file) {
        ...
        /* 
            ...
            {"MIME list" : https://www.freeformatter.com/mime-types-list.html}
